...
I want to analyze on all three consoles together. The best way to do that is to join my data frames together. There are multiple joins avaiable to me in R. Considering that they have the exact same variables, it  makes the most sense to create  a new dataframe using bind_rows. I originally created in by combining  WiiU_df with the PS4_df and then the combined dataframe with the Xbox1_df. However I've recently learned that I can use the purrr package for this. The reduce function allows me to apply a dplyr function to a list. So I'll do just that. If you notice, jag and Big3_df gave me the same results.    
